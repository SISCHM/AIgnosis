...
# Funktion zum Erstellen von Frage-Antwort-Paaren aus einem Dialog
def extract_qa_pairs(dialogue):
    ...
    for i in range(len(dialogue_lines) - 1):
        if "Doctor:" in dialogue_lines[i] and "Patient:" in dialogue_lines[i + 1]:
            ...
            qa_pairs.append({"prompt": f"Doctor: {question}", "completion": f"Patient: {answer}"})
            ...
            qa_pairs.append({"prompt": f"Patient: {question}", "completion": f"Doctor: {answer}"})
...
with open('Data/qa_pairs.jsonl', 'w') as outfile:
